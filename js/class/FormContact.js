...
const formValidate = (entries) => {
    ...
    Object.entries(entries).forEach( ([key, value]) => { // eslint-disable-line
        if(!value.validate){
            value.controlFunc()
            formCompleted = false
        }
...
const setError = (elem, error) => {
    let target = NodeList.prototype.isPrototypeOf(elem) ? elem[0].parentNode : elem.parentNode // eslint-disable-line
